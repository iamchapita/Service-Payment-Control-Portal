@isset($inputsName)
    @for ($i = 1; $i <= $inputsName; $i++) <br>
        ...
        <div class="col-md-6 col-xl-3">
            <div class="input-group mb-3">
                ...
                <select name="userInput{{ $i }}" class="form-select" id="userInput{{ $i }}" required
                    autocomplete="off">
                    <option selected value="">Seleccione un Usuario</option>
                    @foreach ($users as $user)
                        @if (isset($values))
                            <option
                                @foreach ($values as $value) @if ($value->idUserFK == $user->id)
                    selected
                    @endif @endforeach
                                value="{{ $user->id }}">
                                {{ $user->texName }}
        ...
        <div class="col-md-6 col-xl-3">
            <div class="input-group mb-3">
                ...
                <select class="form-select" name="serviceInput{{ $i }}" id="serviceInput{{ $i }}"
        ...
        <div class="col-md-6 col-xl-3">
            <div class="input-group mb-3">
                ...
                <select class="form-select" name="monthInput{{ $i }}" id="monthInput{{ $i }}"
                    required autocomplete="off">
                    <option selected value="">Seleccione un Mes</option>
                    @foreach ($months as $month)
                        @if (isset($values))
                            <option
                                @foreach ($values as $value) @if ($value->idMonthFK == $month->id)
                    selected
                    @endif @endforeach
                                value="{{ $month->id }}">
                                {{ $month->texName }}
        ...
        <div class="col-md-6 col-xl-4">
            <div class="input-group mb-3">
                ...
                <select class="form-select" name="depositStatus{{ $i }}" id="depositStatus{{ $i }}"
                    required autocomplete="off" onchange="checkDepositeStatus({{ $i }})">
                    <option value="">Seleccione el Estado</option>
                    <option value="0"
                        @if (isset($values)) @foreach ($values as $value) @if ($value->boolDeposited == 0)
                    selected @endif
                        @endforeach
    @endif
    >
    ...
    <option value="1"
        @if (isset($values)) @foreach ($values as $value) @if ($value->boolDeposited == 1)
                    selected @endif
        @endforeach
        @endif
        >
    ...
    <div class="col-md-12 col-xl-4">
        <div class="input-group mb-3" id="dateField">
            ...
            <input type="date" @if (!isset($values)) disabled @endif
                @if (isset($values)) @foreach ($values as $value) @if ($value->boolDeposited == 0)
            disabled @endif
                @endforeach
            @endif class="date form-control" id="depositDateInput{{ $i }}"
            name="depositDateInput{{ $i }}"
            @if (isset($values)) @foreach ($values as $value) value={{ $value->datDepositedDate }} @endforeach @endif
            autocomplete="off">
        </div>
    </div>
    @endfor
@else
    ...
    <div class="col-md-6 col-xl-4">
        <div class="input-group mb-3">
            ...
            <select class="form-select" name="depositStatus1" id="depositStatus1" required autocomplete="off"
                onchange="checkDepositeStatus(1)">
                <option value="">Seleccione el Estado</option>
                <option value="0"
                    @if (isset($values)) @foreach ($values as $value) @if ($value->boolDeposited == 0)
                    selected @endif
    ...
    <div class="col-md-12 col-xl-4">
        <div class="input-group mb-3" id="dateField">
            ...
            disabled @endif
                @endforeach
            @endif class="date form-control" id="depositDateInput1" name="depositDateInput1"
            @if (isset($values)) @foreach ($values as $value) value={{ $value->datDepositedDate }} @endforeach @endif
            autocomplete="off">
        </div>
    </div>
